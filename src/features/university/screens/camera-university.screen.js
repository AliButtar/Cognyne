...
export const UniversityCameraScreen = ({ navigation, route }) => {
  ...
  const { onUniversityCreate, universityError } = useContext(UniversityContext);

  const { universityName, code, universityStudentsDetails } = route.params;
  ...
  const snap = async () => {
    if (cameraRef) {
      ...
      if (!universityStudentsDetails) {
        const flag = await compareFaceEncodingWithUser(user, faceEncoding);

        if (flag) {
          onUniversityCreate(universityName, code);
