...
export const RegisterCameraScreen = ({ navigation, route }) => {
  ...
  const { getFaceEncoding } = useContext(FaceVerificationContext);
  ...
  const snap = async () => {
    if (cameraRef) {
      ...
      const data2 = { faceEncoding: faceEncoding, ...data };
      //   console.log(data2.email);

      await onRegister(email, password, repeatedPassword, data2);
