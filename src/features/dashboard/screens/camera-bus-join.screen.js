...
export const BusJoinedCameraScreen = ({ navigation, route }) => {
  ...
  const { updateBusMemberVerifiedStatus, error } = useContext(BusContext);

  const { bmBusCode, bmId } = route.params;
  ...
  const snap = async () => {
    if (cameraRef) {
      ...
      if (flag) {
        updateBusMemberVerifiedStatus(bmBusCode, bmId);
      }
