...
export const UniversityJoinedCameraScreen = ({ navigation, route }) => {
  ...
  const { updateUniversityVerifiedStatus, universityError } =
    useContext(UniversityContext);

  const { stdUniversityCode, stdId } = route.params;
  ...
  const snap = async () => {
    if (cameraRef) {
      ...
      if (flag) {
        updateUniversityVerifiedStatus(stdUniversityCode, stdId);
      }
